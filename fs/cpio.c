...
uintptr_t *initrd_start = 0x04200000UL + VM_START; //0x80000000+0x4200000
int cpio_header_size = 14 * 8 - 2;

size_t getFieldVal(char *field, int length)
{
    //cpio_new_header里面的数组八位转化成unsigned long 的八位十六机制的数，如0x88888888，左边第一位对应第[0]位
    size_t value = 0;
    for (int i = 0; i < length; i++)//八位
    {
        switch (field[i])
        {
        case '0':
...
struct cpio_stat cpio_find_file(char *name)
{
    ...
    while (check_file_name("TRAILER!!!", p->c_name))//while不相等
    {
        printk("Current is: %s\n", p->c_name);

        if (!check_file_name(name, p->c_name))//如果相等
        {
...
{
    struct inode *n = (struct inode *)kalloc(); // kalloc改成了kmalloc
    struct cpio_stat *stat = (struct cpio_stat *)kalloc();
    n->i_private = stat;
...
int readi(struct inode *ip, int user_dst, void *dst, uint off, uint n)
{
    ...
    void *base = stat->data;
    memmove(dst, base + off, n);//在string.c,把从base+off开始复制n个到dst上
    return n;
}
